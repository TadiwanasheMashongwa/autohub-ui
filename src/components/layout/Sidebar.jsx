...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 h-screen bg-brand-dark border-r border-white/5 flex flex-col fixed left-0 top-0 z-40">
      {/* Brand Header */}
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
        ...
        {/* Global Shopping Cart Button (Customer Only) */}
        {user?.role === 'CUSTOMER' && (
          <button
            onClick={() => setIsDrawerOpen(true)}
            className="w-full flex items-center justify-between px-4 py-3 rounded-xl text-slate-400 hover:bg-white/5 hover:text-white transition-all group mt-2"
          >
            <div className="flex items-center gap-3">
      ...
      <div className="p-4 border-t border-white/5 bg-black/20">
        <div className="flex items-center gap-3 px-2 mb-4">
          <div className="h-8 w-8 rounded-full bg-brand-accent/10 border border-brand-accent/20 flex items-center justify-center font-bold text-brand-accent uppercase">
            {user?.email?.[0] || 'U'}
          </div>
          <div className="flex flex-col truncate">
            <span className="text-xs text-white font-bold truncate">{user?.email}</span>
            <span className="text-[9px] text-brand-accent font-mono uppercase tracking-tighter">
              {user?.role} MODE
            </span>
          </div>
        </div>
