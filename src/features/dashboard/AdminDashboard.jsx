    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="relative w-full md:w-80">
          <Search className="absolute left-3 top-3 h-4 w-4 text-slate-500" />
          <input 
            type="text" placeholder="Search Matrix..." value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-slate-900 border border-white/10 p-2.5 pl-10 rounded-xl text-white text-sm outline-none focus:border-brand-accent"
          />
        </div>
        {!isAdding && (
          <button onClick={() => setIsAdding(true)} className="flex items-center gap-2 px-4 py-2.5 bg-brand-accent text-brand-dark font-black text-[10px] uppercase rounded-lg">
      ...
      {isAdding && (
        <div className="bg-white/5 border border-brand-accent/30 p-6 rounded-2xl animate-in slide-in-from-top-2">
          ...
          <div className="flex justify-end gap-4 mt-6">
            <button onClick={() => setIsAdding(false)} className="text-slate-500 uppercase text-[10px] font-black">Abort</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredVehicles?.map(v => (
          <div key={v.id} className="bg-white/5 border border-white/10 p-5 rounded-2xl flex justify-between items-center group">
            ...
            <button onClick={() => { if(window.confirm('Delete vehicle?')) deleteMutation.mutate(v.id); }} className="text-slate-700 group-hover:text-red-500 transition-colors p-2"><Trash2 size={18}/></button>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
          <div className="flex justify-between mb-4">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="bg-slate-900 border border-white/10 pl-7 pr-3 py-1.5 rounded-lg text-[10px] text-white"
          ...
          <div className="max-h-96 overflow-y-auto space-y-2">
            {clerks?.map(c => (
              <div key={c.id} className="flex justify-between items-center p-3 border border-white/5 rounded-lg hover:bg-white/[0.02]">
