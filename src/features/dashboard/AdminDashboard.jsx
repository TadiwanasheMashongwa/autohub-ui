  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <div className="flex justify-between items-center gap-4">
        <div className="relative w-full md:w-80"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500"/><input type="text" placeholder="Search SKU/Name..." value={q} onChange={(e) => setQ(e.target.value)} className="w-full bg-slate-900 border border-white/10 p-2.5 pl-10 rounded-xl text-white text-sm" /></div>
          ...
          <tbody className="text-xs">
            {partsData?.content?.map(p => (
              <tr key={p.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                ...
                <td className="p-4 text-slate-400">{p.sku} | {p.brand}</td>
  ...
  return (
    <div className="fixed inset-0 bg-brand-dark/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-2xl p-8 space-y-6">
        ...
        <input type="text" placeholder="Find vehicle..." value={vQuery} onChange={e => setVQuery(e.target.value)} className="w-full bg-white/5 border border-white/5 p-4 rounded-xl text-white outline-none" />
        <div className="max-h-64 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-3">
          {filteredV?.map(v => (<button key={v.id} onClick={() => mapMutation.mutate(v.id)} className="p-4 bg-white/5 rounded-xl text-left hover:bg-brand-accent hover:text-brand-dark transition-all"><p className="font-bold text-xs">{v.make} {v.model}</p><p className="text-[10px] opacity-60">{v.yearRange}</p></button>))}
        </div>
      </div>
    </div>
  );
...
function CategoryManager() {
  ...
  return (
    <div className="space-y-6">
      <div className="relative w-80"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500"/><input type="text" placeholder="Filter..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-slate-900 border border-white/10 p-2 pl-10 rounded-xl text-sm" /></div>
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
        <table className="w-full text-left"><thead className="bg-white/5 text-[10px] uppercase text-slate-400 font-bold"><tr><th className="p-4">Name</th><th className="p-4">Description</th><th className="p-4 text-right">Action</th></tr></thead>
          <tbody className="text-xs">{categories?.map(cat => (<tr key={cat.id} className="border-b border-white/5">
            <td className="p-4 font-bold">{cat.name}</td><td className="p-4 opacity-60">{cat.description}</td>
            <td className="p-4 text-right"><button onClick={() => deleteMutation.mutate(cat.id)} className="text-red-400 hover:text-red-500"><Trash2 size={16}/></button></td>
          </tr>))}</tbody>
        </table>
      </div>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {vehicles?.map(v => (
        <div key={v.id} className="bg-white/5 border border-white/10 p-5 rounded-2xl flex justify-between items-center group">
          <div><p className="font-black uppercase text-sm">{v.make} <span className="text-brand-accent italic">{v.model}</span></p><p className="text-[10px] text-slate-500 uppercase mt-1">{v.yearRange}</p></div>
...
function CustomerDirectory() {
  ...
  const exportToCSV = () => {
    ...
    const rows = customers.map(c => `${c.id},"${c.firstName} ${c.lastName}","${c.email}",${c.orderCount},${c.totalSpent}`).join("\n");
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end"><button onClick={exportToCSV} className="flex items-center gap-2 px-6 py-2.5 bg-white/5 border border-white/10 text-[10px] uppercase rounded-lg"><Download size={14}/> Export</button></div>
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden"><table className="w-full text-left"><thead className="bg-white/5 text-[10px] uppercase text-slate-400 font-bold"><tr><th className="p-4">Name</th><th className="p-4">Orders</th><th className="p-4">LTV</th></tr></thead>
        <tbody className="text-xs">{customers?.map(c => (<tr key={c.id} className="border-b border-white/5"><td className="p-4 font-bold">{c.firstName} {c.lastName}</td><td className="p-4">{c.orderCount}</td><td className="p-4 text-brand-accent font-black">${Number(c.totalSpent).toFixed(2)}</td></tr>))}</tbody>
      </table></div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white/5 border border-white/10 p-6 rounded-2xl max-w-xl">
        <h3 className="text-xs font-black uppercase mb-4">Registry</h3>
