    ...
    <div className="max-w-6xl mx-auto p-6 lg:p-12 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cart.items.map((item) => (
            <div key={item.id} className="bg-white/5 border border-white/5 p-6 rounded-2xl flex flex-col sm:flex-row items-center gap-6 group hover:border-brand-accent/20 transition-all">
              <div className="h-24 w-24 bg-black/40 rounded-xl flex-shrink-0 flex items-center justify-center overflow-hidden">
                {item.part.imageUrl ? (
                  <img src={item.part.imageUrl} alt={item.part.name} className="object-cover w-full h-full" />
                ) : (
              ...
              <div className="flex-1 space-y-1 text-center sm:text-left">
                <h3 className="text-white font-bold uppercase tracking-tight">{item.part.name}</h3>
                <p className="text-[10px] text-slate-500 font-mono uppercase tracking-widest">SKU: {item.part.sku}</p>
                <p className="text-brand-accent font-black text-lg">${item.part.price.toFixed(2)}</p>
              </div>

              {/* CHECKLIST: Quantity Controls */}
              <div className="flex items-center gap-4 bg-black/40 p-1 rounded-xl border border-white/5">
                ...
                <span className="text-white font-mono font-bold w-8 text-center">{item.quantity}</span>
                <button 
                  onClick={() => updateQuantity(item.id, item.quantity + 1)}
                  disabled={loading}
                  className="p-2 text-slate-400 hover:text-white disabled:opacity-30"
        ...
        <div className="space-y-6">
          <div className="bg-white/5 border border-white/10 p-8 rounded-2xl sticky top-24 space-y-6">
            <h3 className="text-white font-black uppercase text-sm tracking-widest border-b border-white/5 pb-4">Order Summary</h3>
            ...
            <div className="pt-6 border-t border-white/5 flex justify-between items-end">
              ...
              <span className="text-3xl font-black text-white tracking-tighter">${cart.totalPrice.toFixed(2)}</span>
            </div>
