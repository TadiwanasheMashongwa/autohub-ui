...
export default function MFAVerify() {
  ...
  useEffect(() => {
    // Redirect if someone tries to access this page directly without a temp session
    if (!tempToken) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      // Redirect based on role logic in App.jsx or to a default dashboard
      navigate('/');
    } catch (err) {
      // Error handled by the global axios interceptor toast bridge
      ...
      <div className="max-w-md w-full space-y-8 bg-white/5 p-8 rounded-2xl border border-white/10 shadow-2xl">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Enter the code sent to <span className="text-slate-200">{userEmail || 'your email'}</span>
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleVerify}>
          ...
          <button
            ...
            className="w-full flex justify-center py-3 px-4 rounded-lg text-brand-dark bg-brand-accent font-bold hover:bg-teal-400 disabled:opacity-50 transition-all uppercase tracking-widest text-xs"
          >
